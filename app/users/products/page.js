 ...
 *   stock?: number,
 * }} Product
 * @typedef {Record<string, number>} CartMap
 * @typedef {Record<string, boolean>} BoolMap
 */
...
function loadCart() {
  try {
    const saved = /** @type {Array<{id: string | number, quantity: number}>} */ (
      JSON.parse(localStorage.getItem("cart") || "[]")
    );
    const cartMap = /** @type {CartMap} */ ({});
    saved.forEach(item => {
      if (item && item.id != null) {
        cartMap[String(item.id)] = Number(item.quantity) || 0;
      }
    });
    return cartMap;
  } catch {
...
function ProductsContent() {
  ...
  const [cart, setCart] = useState(/** @type {CartMap} */ ({}));
  const [loading, setLoading] = useState(true);
  const [wishlist, setWishlist] = useState(/** @type {BoolMap} */ ({}));
  const [imgErrors, setImgErrors] = useState(/** @type {BoolMap} */ ({}));
          ...
          {loading ? (
            ...
            <div className="products-grid">
              {filtered.map((product, index) => {
                ...
                const emojiKey = /** @type {keyof typeof EMOJI_MAP} */ (product.category);
                return (
                  <div key={product.id} className="product-card">

                    <div className="product-img-wrap">
                      {product.image_url && !imgErrors[productId] ? (
                        <Image
                          ...
                          onError={() => setImgErrors(prev => ({ ...prev, [productId]: true }))}
                        />
                      ) : (
                        <div style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "8px" }}>
                          <span style={{ fontSize: "80px" }}>{EMOJI_MAP[emojiKey] || EMOJI_MAP.default}</span>
                    ...
                    <div className="product-info">
                      ...
                      {cart[productId] ? (
                        <div className="cart-stepper">
                          <button onClick={() => removeFromCart(productId)} className="stepper-btn">−</button>
                          <span className="stepper-count">{cart[productId]}</span>
                          <button onClick={() => addToCart(productId)} className="stepper-btn add">+</button>
                        </div>
                      ) : (
                        <button onClick={() => addToCart(productId)} className="add-to-cart-btn">
                          Add to Cart
                        </button>
                      )}
                    </div>
